...
KyrieViolinoI = {
  \relative c' {
    ...
    fis8 d\p g,16.-\critnote fis32 g16. g,32 d'8 d d16( c) d d
...
GloriaViolinoI = {
  \relative c' {
    ...
    a, a' a32( g) a16 a, a' g, g' fis, fis' fis32( e) fis16 fis, fis' d, d'
...
CredoViolinoI = {
  \relative c' {
    ...
    a16 f b d, c8 a' \tempoEtIncarnatus b, d d d
    des c16 des des8 des des4 c8 b
    b4 a! a8 a a a %10
    b8. b16 b8 b as as ges ges
    f f f es es4 \tempoCrucifixus r8 b'
...
SanctusViolinoI = {
  \relative c' {
    ...
    R1*2
    es'4\fE es es8 f16 es d8 c
    d4 d8 c b2~
    b8 b a4 b b %5
    b4. b8 a4 \tempoPleni c16 c c c
    f c c c f c c c d b b b c f f f
    d b c f f c a f d' b f d f' f f f
    b f f f b f f f g es es es f b b b
    g es f b b f d b g' es b g b' b, b b %10
...
AgnusViolinoI = {
  \relative c' {
    ...
    es4 d8 c d4
    c b2
    a4 f'16(^\critnote c b a) d( b a g) %5
    c( a g a) f4 c'
    h4. h8 h h
    c c c4 h
    c c'16( g f es) as( f es d)
    \once \slurDashed g( es d es) es,4 d\trillE %10
    c es' es
    es es es
    d d8 d c4
    b c2
    b4 g'16( d c b) \slurDashed es( c b a) %15
    d( b a b) \slurSolid g4 b
    b4. b8 a cis
    d d d4 cis
    d \slurDashed d'16( a g f) b( g f e)
    a( f e f) \slurSolid f,4 e %20
    d d' d
    d d d
    es! es8 es es4
    d es2
    d4 b'16( f es d) g( es d c) %25
    d( b a b) b,4 r
    r g''16( es d es) \once \slurDashed es( b as b)
    g4 r r
    r a'!16( f e f) f( c b c)
    a4 r r %30
    R2.
    r4 b'16( f es d) g( es d c)
    d( b a b) d,4 c
    b r r
    r b''16(\pE f es d) f( d c b) %35
    g4 b'16( f es d) f( d c b)
    g2.\fE
